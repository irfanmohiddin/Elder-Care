...
if (!volunteerData) {
  ...
  async function fetchProfile() {
    ...
    try {
      const res = await fetch(`http://localhost:3000/get-volunteer?email=${email}`);
  ...
  async function loadRequests(page = 1) {
    ...
    const res = await fetch(`http://localhost:3000/requests?category=${category}&location=${location}`);
  ...
  async function loadAcceptedTasks() {
    const res = await fetch(`http://localhost:3000/requests?assignedTo=${volunteer._id}&status=assigned`);
  ...
  window.handleAction = async function (requestId, action) {
    const res = await fetch("http://localhost:3000/request-action", {
      method: "POST",
